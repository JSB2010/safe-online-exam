...
- Students require the Canvas Developer Key scope `url:GET|/api/v1/login/session_token`; the first course-navigation launch opens the one-time Canvas connection screen before the student dashboard is available
...
- `SEB_QUIT_PASSWORD`
- `SEB_REQUIRED_DOMAINS`, concrete school-reviewed resource hostnames only; wildcards and identity-provider hosts are rejected.
...
For a new school, use [docs/school-deployment.md](docs/school-deployment.md) with `cloudbuild-school.yaml`. See [docs/deployment.md](docs/deployment.md) for maintained target details and [docs/canvas-school-setup.md](docs/canvas-school-setup.md) for Canvas LTI installation and theme JavaScript setup. The deployed service also provides a public, read-only role handoff at `${TOOL_URL}/setup`; it links to public health, LTI, and JWKS checks but cannot verify Canvas-side installation or theme loading.
...
Instructors manage tools in **Course settings → Exam tools**, alongside the **Security** and **Allowed URLs** tabs. Every new course starts with four disabled, editable preloaded tools: Desmos Graphing Calculator (`https://www.desmos.com/calculator`), Desmos Scientific Calculator, GeoGebra Graphing Calculator, and GeoGebra Scientific Calculator. They are ordinary course records after seeding: an instructor can rename, retarget, edit resources, or remove them, and removed tools are never restored automatically. All tools require an exact HTTPS launch URL plus visible exact-page or path entries. A whole-domain rule requires an explicit confirmation and is visibly marked broad; identity-provider hosts, wildcard hosts, credentials, and caller-authored regular expressions are rejected. Everything else, including sign-in, saved work, sharing, and unrelated paths, stays blocked in SEB.

Quiz settings contain only the **Exam tools** checkbox list. A quiz normally inherits course-default tool toggles; checking a box creates a tool-selection override, and **Reset to course defaults** removes that override. Course catalog changes update the live definitions for every assessment, while an override retains only its selected IDs. Only selected tools add launch/resource rules to the `.seb` file, Config Key proof state, proof response, or student sidebar. Any change affecting selected tools invalidates the old configuration, so students must download a fresh `.seb` file.

For existing Firestore data, preview the idempotent migration before deploying the feature, then apply it once against the intended environment:
...
The migration seeds only pre-catalog courses with the four preloaded tools, converts the retired Desmos ACT/testing preset to standard Desmos, promotes legacy quiz-only custom tools as disabled course tools while retaining their quiz selections, and clears stale Config Key state. It never restores an intentionally removed tool from an initialized catalog. If an exit password is lost, disable affected assessments, set a replacement course or managed server password, re-enable them, and issue fresh configs; do not replace native early-quit protection with a static passwordless URL. The proof-gated student sidebar is draggable, collapsible with a keyboard-operable button, viewport-clamped when restored, and reuses/focuses one window per tool. It is only a launcher; the SEB URL filter allowlist remains the enforcement mechanism for which external sites can load.
