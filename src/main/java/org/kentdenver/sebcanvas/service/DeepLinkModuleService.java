...
public class DeepLinkModuleService {
    ...
    private ModuleItemInfo findModuleItemForQuiz(String courseId, Quiz quiz, String userId, JsonNode modules) {
        ...
        try {
            ...
            // Search for existing module item pointing to this quiz
            ModuleItemInfo existingItem = findModuleItemForQuiz(courseId, quiz, userId, modules);

            if (existingItem != null) {
                log.info("Found existing module item for quiz {} in module {}", quiz.getId(), existingItem.moduleId);

                if (sebRequired) {
                    // Update existing item to use LTI Resource Link
                    return updateModuleItemToLtiLink(courseId, existingItem, quiz, userId);
                } else {
                    // Restore to direct Canvas quiz link
                    return restoreModuleItemToDirectLink(courseId, existingItem, quiz, userId);
                }
            } else {
                log.warn("No existing module item found for quiz: {}. Instructor should add quiz to module first.", quiz.getId());
                // We don't auto-create module items - instructor must place quiz in module first
                return false;
            }

        } catch (Exception e) {
            log.error("Error creating/updating module item for quiz {}: {}", quiz.getId(), e.getMessage(), e);
            return false;
        }
    }
    ...
    private ModuleItemInfo findModuleItemForQuiz(String courseId, Quiz quiz, String userId, JsonNode modules) {
        try {
            for (JsonNode module : modules) {
                ...
                // Look for item matching this quiz
                for (JsonNode item : items) {
                    String itemId = item.get("id").asText();
                    String type = item.has("type") ? item.get("type").asText() : "";
                    String contentId = item.has("content_id") ? item.get("content_id").asText() : "";

                    // Check if this is our quiz (either as Quiz type or ExternalTool type)
                    boolean isMatchingQuiz = false;

                    if ("Quiz".equalsIgnoreCase(type) && contentId.equals(quiz.getCanvasQuizId())) {
                        isMatchingQuiz = true;
                    } else if ("ExternalTool".equalsIgnoreCase(type)) {
                        // Check if external tool URL points to our app with this quiz ID
                        String externalUrl = item.has("external_url") ? item.get("external_url").asText() : "";
                        if (externalUrl.contains("/seb/launch/" + quiz.getId())) {
                            isMatchingQuiz = true;
                        }
                    }
                    ...
                    if (isMatchingQuiz) {
                        ...
                        info.moduleId = moduleId;
                        info.itemId = itemId;
                        info.title = item.has("title") ? item.get("title").asText() : quiz.getTitle();
                        info.type = type;
                        return info;
                    }
                }
            ...
            log.error("Error finding module item for quiz {}: {}", quiz.getId(), e.getMessage());
            return null;
        }
    }
