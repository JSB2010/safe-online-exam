...
| LTI and OAuth                          | LTI/OAuth controller and service tests                      | Signed-claim validation, browser binding, state replay prevention, role routing, OAuth state binding, cross-site initiation rejection, exact-origin popup completion, non-privileged callback fallback, scope-profile upgrades, and token ownership. |
...
| SEB configuration and proof            | `seb-*.test.ts`                                             | Plist generation, required assessment encryption, plaintext fail-closed behavior, Config Key validation, configuration grants, proof redemption, handoff records, exit grants, and password rules.                                                   |
