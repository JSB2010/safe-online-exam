...
- Start SEB with an empty Canvas cookie jar. Confirm Canvas `/login` and the exact Google identity endpoints load, SSO returns to the assessment, and unrelated Canvas course paths or wildcard Google/Googleusercontent hosts remain blocked.
