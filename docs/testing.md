...
- Course-level SEB defaults, persisted four-preset exam-tool catalogs, quiz default-versus-override selections, and structured URL rule normalization.
- Exam-tool migration dry-run/apply transformations, canonical legacy Desmos conversion, custom-tool selection preservation, strict custom resource validation, explicit broad-domain confirmation, and idempotence.
- Content discovery and New Quiz SEB setting behavior.
- Quiz service enable/disable and access-code behavior.
- SEB configuration plist generation, explicit cross-platform lockdown keys, canonical URL filter keys, exact selected-tool launch/resource rules, exact-query pinning, and allowed-domain policy.
...
25. Run `npm run migrate:exam-tools -- --dry-run` against a production snapshot before the release, review the reported course/assessment counts, then run `npm run migrate:exam-tools -- --apply` once in the intended environment. Confirm every course has the four preset cards, legacy Desmos resolves to standard `/calculator`, custom migrated tools are disabled by default but remain checked on their original quizzes, and students with pre-migration files must download a fresh configuration.
