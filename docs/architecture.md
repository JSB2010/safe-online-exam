...
The client router loads the React view module selected by the server bootstrap instead of shipping every administrator, instructor, student, and SEB workflow in the entry bundle. Route chunks and the shared React runtime use content-hashed filenames with immutable caching; the small stable entry and stylesheet continue to revalidate. The app shell reads Vite's build manifest to preload the shared runtime without adding a first-load request waterfall. Public JavaScript and CSS responses are compressed, while user-specific HTML and API responses remain outside that compression middleware.

The public detector remains one JavaScript response at `/js/canvas-seb-detector.js` (with its compatibility alias), but its maintainable source is assembled in `manifest.json` order during development, tests, and the server build. Production serves and reuses the minified assembled template behind the same route, then injects the deployment base URL and diagnostic flags per response. The Canvas theme loader already limits that download to supported assessment routes whose authoritative requirement check returns `sebRequired: true`; splitting the detector into extra browser requests would lose cross-fragment compression and add startup ordering risk.
...
Every Canvas OAuth connection requests the same complete application scope set, including the course-list permission used for teacher-scoped tool duplication (`url:GET|/api/v1/courses`) and the session-token permission required for SEB handoff (`url:GET|/api/v1/login/session_token`). This is an explicit product decision: one durable grant must remain valid when a person is an instructor in one course and a student in another. The tradeoff is that a student-only authorization consents to scopes the same account may need later; Canvas still enforces the user's actual course permissions, and the application never treats scope possession as an LTI role or course-authorization check. The one-time Canvas session URL is generated server-side for each student configuration download. Browser cookies are never copied to the configuration or exposed through the API.

Access and refresh tokens are encrypted before PostgreSQL persistence with an
independent AES-256-GCM keyring. The record ID and Canvas user ID are
authenticated as associated data, while nonsecret scope and identity metadata
remains queryable. Legacy plaintext rows are read-only to support a staged
migration and are rewritten by the explicit maintenance command.
...
Instructor discovery refreshes Classic and New Quiz data from Canvas. Publication is stored as evidence, not a defaulted boolean: Classic Quizzes retain the quiz value, while New Quizzes retain both the Assignment wrapper and New Quiz values. Agreement is authoritative, disagreement is a conflict, one valid source is degraded single-source evidence, and absent or malformed values remain unknown. The compatibility `published` field is `null` for conflict or unknown. Legacy boolean-only rows remain unknown until refreshed.

Global readiness combines that evidence with discovery freshness, Canvas unlock/lock dates, and complete SOE settings. It drives instructor preflight and administrator issue reporting, but it does not authorize a student. A pure-student configuration grant performs a read-only Canvas inventory call with that learner's existing OAuth identity, matches the exact course and assessment, and enforces explicit publication and date boundaries. Student-visible objects with an omitted publication property are accepted because Canvas does not list unpublished work to students; the incomplete response is still recorded in structured diagnostics. Results coalesce only for the same learner/course/content tuple. Successful visibility remains cached for 30 seconds, while an unavailable result remains cached for only two seconds so a teacher's just-in-time publication or differentiated-assignment change becomes visible promptly. Failed upstream requests are not cached. Per-instance Canvas verification concurrency is bounded, and upstream overload, timeout, rate limit, malformed response, or server failure is retryable rather than converted to absence.
...
Student downloads do not use a reusable `.seb` link. A verified LTI principal requests a configuration grant, and the server first verifies the exact assessment through that learner's Canvas identity. A success mints a non-identifying launch-admission digest with a five-minute lifetime, then a one-time 120-second capability bound to the principal, course, content ID, current settings fingerprint, and admission. `GET /seb/config/:courseId/:contentId.seb` consumes that capability.
...
On the Canvas assessment page, the detector gets the SEB Config Key hash and current browser URL through the SEB JavaScript API. It requests a proof from `POST /api/seb/access-proof/:courseId/:quizId`; the server verifies the assessment, current settings fingerprint, URL family, Config Key hash, and unexpired launch admission before returning a one-time proof token. The admission follows the configuration grant, download, dynamic handoff, Config Key proof, and access-code release without consulting contradictory course-wide publication state again. Settings changes, disable, or course reset still invalidate every boundary; admission expiry requires a fresh Canvas launch.
...
In an ordinary browser, an access-code field is only a Canvas challenge signal. Before showing the SEB-required prompt, the detector calls `GET /api/seb/requirement/:courseId/:quizId`. The endpoint normalizes the assessment identifier and performs one exact `assessments` primary-key lookup; a short, bounded promise cache coalesces concurrent checks for the same assessment. `sebRequired` reports whether SOE is configured, so a stale global publication cache cannot suppress the learner launch path. The additive `globallyReady` diagnostic reports instructor preflight state. Actual authorization occurs only at the learner-scoped configuration grant.
...
| `GET /seb/launch-handoff`                                                                         | Consume a short-lived server-backed browser handoff and render the same-tab launcher. |
...
The server derives the browser handoff return target from the verified Canvas
course. Assessment and setup-check clients do not supply that target. Consuming
a handoff removes access to its `sebs:` configuration URL, while the short-lived
record may still expose only its validated Canvas course return target so a
reload or reused link can exit cleanly. Browser return actions replace the
launcher history entry to prevent a back-navigation loop through the
SEB-required page.
