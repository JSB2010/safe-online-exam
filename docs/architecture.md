...
The generated URL filter keeps assessment navigation scoped to the configured Canvas course/content paths. It also permits the configured Canvas host's `/login` family because a fresh SEB cookie jar must complete Canvas authentication before returning to the assessment, and permits exact Google identity/resource hosts needed for school SSO. No wildcard Google, Canvas, Instructure, or user-content host is added.
