...
An administrator course reset performs strict Classic Quiz and New Quiz discovery, removes each Canvas access code with the account-administrator grant, and only then deletes course-related transient state, assessments, the course policy, and per-course school-tool preset assignments in one PostgreSQL transaction. The shared OAuth grant and root-account course connection are deliberately retained. If a Canvas response is lost, a later Canvas mutation fails, or the database transaction fails, the service restores every access code that may have changed and its prior local assessment record in reverse order. A failed compensation or lost lease is reported as an indeterminate result that requires refresh and verification.
