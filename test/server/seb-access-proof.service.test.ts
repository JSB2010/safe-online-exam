...
describe("SebAccessProofService", () => {
  it("mints single-use tokens bound to course, content, and config generation across service instances", async () => {
    ...
    const token = await firstInstance.mintProof("course-1", "quiz-1", "generation-1");

    await expect(secondInstance.consumeProof(token, "course-2", "quiz-1", "generation-1")).resolves.toBe(false);
    await expect(firstInstance.consumeProof(token, "course-1", "quiz-1", "generation-1")).resolves.toBe(false);

    const stale = await firstInstance.mintProof("course-1", "quiz-1", "generation-1");
    await expect(secondInstance.consumeProof(stale, "course-1", "quiz-1", "generation-2")).resolves.toBe(false);

    const valid = await firstInstance.mintProof("course-1", "quiz-1", "generation-2");
    await expect(secondInstance.consumeProof(valid, "course-1", "quiz-1", "generation-2")).resolves.toBe(true);
    await expect(firstInstance.consumeProof(valid, "course-1", "quiz-1", "generation-2")).resolves.toBe(false);
  });

  it("rejects expired proof tokens", async () => {
    const repositories = createInMemoryRepositories();
    const service = new SebAccessProofService({ value: repositories } as RepositoryProvider);
    const token = await service.mintProof("course-1", "quiz-1", "generation-1");
    ...
    await expect(service.consumeProof(token, "course-1", "quiz-1", "generation-1")).resolves.toBe(false);
  });
  ...
  it("rejects missing proof tokens", async () => {
    ...
    await expect(service.consumeProof(undefined, "course-1", "quiz-1", "generation-1")).resolves.toBe(false);
  });
